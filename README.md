...
拠点でゆっくり注文するUberEatsが横2.5画像というUIを採択している、ここをベースに3画像が多めの情報であり、画面を引き目にスキャンしやすいように色々なサイズを試行錯誤して設計しました。
<dev>
...
違いは、グリッド右端部分に表示される、次の画像のはみ出し部分で、上の部分の上が少なく下のはみ出しが大きく見えるように設計しました。食べたい食事に向き合う際には無意識に見えてしまう上段のはみ出しをほぼ見えないようにして、次の商品がある案内が欲しい下段の商品画像付近にのみにはみ出しを多めに表示させることで、商品情報の取得に集中してる際には邪魔しないのに、スクロール最後の案内が欲しい時だけそっと気づくような案内を実現させようとした設計です。
<dev>
...
- 効果
違いは一番上にある「ルート上の店舗」の表示店舗数の違いです。この目的は購入意思決定のスピード効率化で購入意思の促進になり、購入決定を早める目的もある。
